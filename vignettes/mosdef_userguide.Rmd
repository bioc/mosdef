...
* `geneID`: Which format the genes are provided. If you provide a `DESeqDataset` and `DESeqResults`, then `r BiocStyle::Biocpkg("mosdef")` does it for you and uses symbols. If you provide vectors please specify a value.
...
* `Annotated`: The number of genes in your dataset that are associated with this term (TODO Or is this the number of genes overall associated with this term.Aand then in the col "Significant" is the number of these term associated genes that are significantly enriched in the analyzed dataset )
* `Significant`: The number of significantly enriched genes in your data associated with that term
* `Expected`: The expected number of significant genes associated with that  term based of of the number of genes that were provided.
* `Rank in p.value_classic`: TODO No idea
* `p.value_elim`: FDR of your findings and the parameter we suggest to use for further subsetting of your result
* `p.value_classic`: uncorrected pvalue? TODO
* `genes`: Which genes from your data, associated with that term, were significantly enriched. This column will later on be used to label the genes in the go_volcano function (see below)
...
```
The function will return a table with `r ncol(res_enrich_macrophage_goseq)` columns

* `category`: The computer readable GO term ID.
* `over_represented_pvalue`: FDR of your findings 
* `under_represented_pvalue`: TODO No idea
* `numDEInCat`: The number of genes differentially expressed in your data associated with this term
* `numInCat` The number of genes overall associated with this term
* `term`: The human readable GO term name.
* `ontology`: The ontology analysed (usually BP for Biological process for more info on the different ontologies please consult AMIGO (TODO can we say this here?))
* `p.adj`: FDR of your findings adjusted for multiple testing, and the parameter we suggest to use for further subsetting of your result
* `genes`: Which genes from your data, associated with that term, were significantly enriched as Ensemble IDs. 
* `genesymbols`: Which genes from your data, associated with that term, were significantly enriched as human readable gene symbols. This column could later on be used to label the genes in the go_volcano function (see below)
...
```
The function will return a large enrich result containing some metadata and the enrichment results with `r ncol(res_enrich_macrophage_cluPro@result)` columns (To look at this table in particular run: `View(res_enrich_macrophage_cluPro@result)`).

These columns are:

* `ID`: The computer readable GO term ID.
* `Description`: The human readable GO term name.
* `GeneRatio`: Ratio of significantly enriched genes in your data associated with the term vs significant genes in your data overall
* `BgRatio`: Ratio of genes (significant and non significant) in your data associated with the term vs genes in your data overall TODO (Ok like this?)
* `over_represented_pvalue`: FDR of your findings not corrected for multiple testing
* `p.adjust`: FDR of your findings adjusted for multiple testing, and the parameter we suggest to use for further subsetting of your result
* `qvalue`: TOD (no idea)
* `geneID`: Which genes from your data, associated with that term, were significantly enriched as human readable gene symbols. This column could later on be used to label the genes in the go_volcano function (see below). Note that here they are split with a / not a , so if you use the go_volcano function with a clusterprofiler result ensure to set the `gene_col_seperator`parameter. 
* `Count`: Number of significantly enriched genes in your data associated with the term
...
```{r topgoalt}
res_subset <- deresult_to_df(res_macrophage_IFNg_vs_naive)[1:500, ] # reduce size to increase computational speed
myde <- res_subset$id
...
res_subset <- deresult_to_df(res_macrophage_IFNg_vs_naive, FDR = 0.05)[1:500, ]
